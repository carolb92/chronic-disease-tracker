...
import { evaluateDiabetesHedisMeasures, A1C_GOAL } from "./hedis";
import { makeObservation, makeBPObservation } from "@/test/fixtures";

function findMeasure(
	results: ReturnType<typeof evaluateDiabetesHedisMeasures>,
	name: string,
) {
...
describe("BP control", () => {
	it("is met when the most recent reading is below goal", () => {
		...
		const result = findMeasure(evaluateDiabetesHedisMeasures(obs), "Blood Pressure Control");
	...
	it("uses the most recent reading when multiple exist", () => {
		...
		const result = findMeasure(
			evaluateDiabetesHedisMeasures([older, newer]),
	...
	it("is insufficient-data when no BP reading exists", () => {
		const result = findMeasure(evaluateDiabetesHedisMeasures([]), "Blood Pressure Control");
...
describe("A1c control", () => {
	it("is met when below goal", () => {
		...
		const result = findMeasure(evaluateDiabetesHedisMeasures(obs), "HbA1c Control");
		expect(result?.status).toBe("met");
	});
	...
	it("is insufficient-data when no reading exists", () => {
		const result = findMeasure(evaluateDiabetesHedisMeasures([]), "HbA1c Control");
		expect(result?.status).toBe("insufficient-data");
	});
});
...
describe("Kidney health evaluation", () => {
	it("is met when eGFR and uACR were both performed in the same year", () => {
		...
		const result = findMeasure(evaluateDiabetesHedisMeasures(obs), "Kidney Health Evaluation");
		expect(result?.status).toBe("met");
	});
	...
	it("is insufficient-data when both eGFR and uACR are missing", () => {
		const result = findMeasure(evaluateDiabetesHedisMeasures([]), "Kidney Health Evaluation");
		expect(result?.status).toBe("insufficient-data");
	});
...
describe("evaluateDiabetesHedisMeasures", () => {
	it("returns all four diabetes measures", () => {
		const results = evaluateDiabetesHedisMeasures([]);
